...
Le script `scripts/sync_trello.py` écrit uniquement les champs publics dans `data/collaborations.json`. Le site affiche les projets dans les sections FR et EN. Le workflow GitHub Actions vérifie Trello chaque heure, valide le changement dans Git et déploie le site via GitHub Pages. Pour l'activer, ajouter les secrets de dépôt `TRELLO_API_KEY` et `TRELLO_TOKEN`, ainsi que la variable de dépôt `TRELLO_SYNC_ENABLED` égale à `true` dans Settings → Secrets and variables → Actions. Dans Settings → Pages, régler la source sur **GitHub Actions** ; les commits faits avec `GITHUB_TOKEN` ne déclenchent pas les déploiements Pages configurés « Deploy from a branch ». Conserver le domaine personnalisé `ctrltek.live` et vérifier HTTPS. Le jeton Trello doit pouvoir lire ce tableau ; ne jamais le placer dans le site ni dans Git. On peut aussi lancer manuellement le workflow après configuration.
